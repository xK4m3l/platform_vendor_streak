...
func init() {
	android.RegisterModuleType("streak_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = streakExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = streakExpandVariables(ctx, dep_file)
	...
	cmd := streakExpandVariables(ctx, String(g.properties.Cmd))
