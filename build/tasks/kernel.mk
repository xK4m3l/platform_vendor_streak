...
ifeq ($(TARGET_KERNEL_CLANG_COMPILE),true)
    ...
    KBUILD_COMPILER_STRING := $(shell $(TARGET_KERNEL_CLANG_PATH)/bin/clang --version | head -n 1 | \
	    $(BUILD_TOP)/prebuilts/tools-streak/$(HOST_OS)-x86/bin/perl -pe 's/\(http.*?\)//gs' | sed -e 's/  */ /g')
...
define build-image-kernel-modules-streak
    rm -rf $(2)/lib/modules
...
$(TARGET_PREBUILT_INT_KERNEL): $(KERNEL_CONFIG) $(DEPMOD) $(DTC)
	...
	$(hide) if grep -q '=m' $(KERNEL_CONFIG); then \
			...
			($(call build-image-kernel-modules-streak,$$modules,$(KERNEL_MODULES_OUT),$(KERNEL_MODULE_MOUNTPOINT)/,$(KERNEL_DEPMOD_STAGING_DIR))); \
		fi
