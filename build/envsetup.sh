...
cat <<EOF
Additional ProjectStreak functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- streakgerrit:    A Git wrapper that fetches/pushes patch from/to ProjectStreak Gerrit Review.
- streakrebase:    Rebase a Gerrit change and push it again.
- streakremote:    Add git remote for ProjectStreak Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for ProjectStreak Github.
...
function streakrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "ProjectStreak Gerrit Rebase Usage: "
